...
export default function BudgetSummary() {
  ...
  if (Budgets && Transactions) {
    return (
      <>
        <h1 className={redIf(totalSpent > totalBudgeted, 'center')}>
          {dollarString(totalSpent)} /{' '}
          <span className='secondary-dark'>{dollarString(totalBudgeted)}</span>
        </h1>
        <h2 className='center'>
          {
            [
        ...
        <div className='budget-summary'>
          <div className='bs-text'>
            {!currentBudgets.length && (
              <Link className='secondary-dark' to='/budgets/new'>
                No budgets active for this month. Create a budget to get
                started.
              </Link>
            )}
